...
Imagine following the flow generated by a vector field $X$. As you move along its integral curves, you can ask a natural question: *how quickly does a tensor field $T$ change along this flow?* To make this precise, fix a parameter value $t$ along the flow and compare the “dragged-back” version of the tensor at the flowed point with the tensor at the original point.

For each $t$, define the difference
...
$$
\mathcal{L} _{X} T^r _{s}
= \lim _{ t \to 0 }
\left( \frac{ \Delta _{t} T^r _{s} }{ t } \right) .
$$

So the Lie derivative is a map that takes a vector field $X$ together with an $(r, s)$ tensor field $T$ and returns another $(r, s)$ tensor field describing how $T$ evolves along the flow generated by $X$.


## Properties of Lie Derivatives

The Lie derivative is a **linear operator** that also satisfies the **Leibniz rule**, so it behaves nicely with products. To get a feel for it, let’s start small. When it acts on a scalar field $f$, it simply gives the directional derivative of $f$ along the vector field $V$ :

$$
\mathcal{L} _{V} f = V \langle f \rangle .
$$

For vector fields, things get a bit more interesting. If $V$ and $U$ are vector fields, then the Lie derivative captures how $U$changes as it flows along $V$. This turns out to be just their commutator:

$$
\mathcal{L} _{V} U = \[V, U\] = V^\nu \partial _{\nu} U^\mu - U^\nu \partial _{\nu} V^\mu .
$$
